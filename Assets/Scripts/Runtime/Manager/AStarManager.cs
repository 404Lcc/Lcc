...
namespace Model
{
    public class AStarManager : Singleton<AStarManager>
    {
        ...
        public void InitManager(float radius, LayerMask layer, bool showWall, bool showPath, GameObject map, float ratio = 1)
        {
            ...
            for (int x = 0; x <= w; x++)
            {
                for (int y = 0; y <= h; y++)
                {
                    ...
                    if (wall)
                    {
                        //InitWallGrid(AssetManager.Instance.LoadGameObject("Wall", false, false, this.wall, AssetType.UI, AssetType.Tool), localPosition, showWall);
                    }
                }
            }
            for (int x = -w + 1; x < 0; x++)
        ...
        public void UpdatePath(List<AStarNodeData> endAStarNodeDataList)
        {
            ...
            {
                //InitWallGrid(AssetManager.Instance.LoadGameObject("Path", false, false, path, AssetType.UI, AssetType.Tool), endAStarNodeDataList[i].localPosition, showPath);
            }
        }
        /// <summary>
        /// 生成路径
