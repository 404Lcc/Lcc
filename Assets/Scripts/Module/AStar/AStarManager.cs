...
namespace Model
{
    public class AStarManager : Singleton<AStarManager>
    {
        ...
        public void InitManager(float radius, LayerMask layer, bool isShowWall, bool isShowPath, GameObject map, float ratio = 1)
        {
            this.radius = radius;
            this.layer = layer;
            ...
            w = Mathf.RoundToInt(Util.GetComponent<BoxCollider2D>(map).bounds.size.x * ratio);
            h = Mathf.RoundToInt(Util.GetComponent<BoxCollider2D>(map).bounds.size.y * ratio);
        ...
        public void UpdatePath(List<AStarNodeData> endAStarNodeDataList)
        {
            ...
            {
                Util.SafeDestroy(item.gameObject);
            }
