...
namespace LccModel
{
	public static class Event
	{
		...
		{
			if (_isInitialize)
				throw new Exception($"{nameof(Event)} is initialized !");

			if (_isInitialize == false)
			{
				// 创建驱动器
				_isInitialize = true;
				_driver = new UnityEngine.GameObject($"[{nameof(Event)}]");
				_driver.AddComponent<EventDriver>();
				UnityEngine.Object.DontDestroyOnLoad(_driver);
				Debug.Log($"{nameof(Event)} initalize !");
			}
		}
		...
		public static void Destroy()
		{
			if (_isInitialize)
			{
				...
				Debug.Log($"{nameof(Event)} destroy all !");
			}
		}
