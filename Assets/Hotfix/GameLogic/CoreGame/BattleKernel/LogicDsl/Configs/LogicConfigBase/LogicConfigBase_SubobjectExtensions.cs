...
namespace LccHotfix
{
    public partial class LogicConfigBase
    {
        ...
        /// </summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public SequenceBhvCfg SpawnSubobjectDanmaku(int tid, string targetPosVar, string initPosVar,
            int count = 12, float angle = 360f, float interval = 0f, string bindPoint = null, string audio = null, bool yBlock = true)
        {
            return Seq(
                BeginCall(node =>
                {
                    node.ClearVar<int>("CV_DanmakuIndex");
                }),
                LoopSeq(new List<ICustomNodeCfg>()
                {
                    BeginCall(node =>
                    {
                        var index = node.GetVar<int>("CV_DanmakuIndex", 0);
                        new PosVarCfg(initPosVar, bindPoint).GetVector3(node, out var initPos);
                        new PosVarCfg(targetPosVar).GetVector3(node, out var targetPos);
                        if (yBlock)
                        {
                            targetPos.y = initPos.y;
                        }
                        var rotateAngle = angle / (count - 1) * (index - count * 0.5f);
                        node.SetVar("CV_DanmakuTargetPos", initPos + Quaternion.AngleAxis(rotateAngle, Vector3.up) * (targetPos - initPos));
