...
namespace LccHotfix
{
    ...
    public sealed class SysSyncViewAnimatorSpeed : IExecuteSystem
    {
        ...
        {
            var animator = entity.comView.GetView<IMainAnimatorView>(EViewCategory.MainGameObject)?.GetMainAnimator();
            if (animator == null)
            {
                return;
            }

            animator.speed = GetAnimSpeedRatio(entity)
                             * BattleBulletTimeUtility.GetCompensateRatio(entity, _metaWorld);
        }
