...
namespace LccHotfix
{
    ...
    */
    public class ViewLoaderComponent : LogicComponent
    {
        ...
        private Dictionary<int, bool> _allViewDone = new Dictionary<int, bool>();
        private Dictionary<int, IReceiveLoaded> _allDoneViewDict = new Dictionary<int, IReceiveLoaded>();

        public override void DisposeOnRemove()
        {
            base.DisposeOnRemove();
            _allViewDone.Clear();
            _allDoneViewDict.Clear();
            RecycleViewLoader();
            mainViewData = null;
        }
        ...
        {
            if (_allViewDone.TryGetValue(loaderData.Category, out var result))
            {
                _allViewDone[loaderData.Category] = false;
            }

            if (_allDoneViewDict.TryGetValue(loaderData.Category, out var receiveLoaded))
            {
                _allDoneViewDict.Remove(loaderData.Category);
            }

            ChangeLoaderInternal(ref mainViewData, loaderData);
        }

        private void ChangeLoaderInternal(ref IViewLoader curLoaderData, IViewLoader loaderData)
        {
            ...
            if (subLoaderList != null)
            {
                for (int i = 0; i < subLoaderList.Count; i++)
                {
                    ...
                    ChangeLoaderInternal(ref data, loaderData);
        ...
        private bool TryAddAllDoneDict(IViewLoader loader)
        {
            ...
            if (_allViewDone.TryGetValue(category, out var isDone))
            {
            ...
            {
                _allViewDone.Add(category, false);
            }
        ...
        public void ReceiveLoaded(int category, IReceiveLoaded loaded, LogicEntity owner, ECWorlds world)
        {
            ...
            {
                _allDoneViewDict.Add(category, loaded);
            }
            else
            {
            ...
            if (_allViewDone.TryGetValue(category, out var value))
            {
                _allViewDone[category] = true;
            }

            bool allDone = true;
            var allDoneList = _allViewDone.Values.ToList();
        ...
        private void Deploy(IViewLoader loader, IViewWrapper parentView, ECWorlds world)
        {
            ...
            if (loader.SubLoaderList != null)
            {
                ...
                {
                    Deploy(loader.SubLoaderList[i], curView, world);
                }
            }
        }
