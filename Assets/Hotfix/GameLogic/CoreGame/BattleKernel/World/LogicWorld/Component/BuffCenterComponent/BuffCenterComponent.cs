...
namespace LccHotfix
{
    public sealed class BuffCenterComponent : LogicComponent, IEntityCommandHandler
    {
        ...
        public bool AddNewBuff(BuffGenInfo genInfo)
        {
            ...
            var buffApplyPolicy = Owner?.OwnerWorld?.GetCreationInfo<BattleKernelCreationInfo>()?.BuffApplyPolicy;
            if (cfg != null && buffApplyPolicy != null && buffApplyPolicy.TryBlockBuffApply(Owner, genInfo))
            {
                return false;
            }
            if (cfg == null)
