    ...
    internal sealed partial class UIManager : Module, IUIManager
    {
        ...
        private sealed partial class UIGroup : IUIGroup
        {
            ...
            public void RemoveUIForm(IUIForm uiForm)
            {
                ...
                if (uiFormInfo == null)
                {
                    throw new Exception(string.Format("Can not find UI form info for serial id '{0}', UI form asset name is '{1}'.", uiForm.SerialId, uiForm.UIFormAssetName));
                }
                ...
                {
                    throw new Exception(string.Format("UI group '{0}' not exists specified UI form '[{1}]{2}'.", m_Name, uiForm.SerialId, uiForm.UIFormAssetName));
                }
