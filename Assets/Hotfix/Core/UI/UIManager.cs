    ...
    internal sealed partial class UIManager : Module, IUIManager
    {
        ...
        public int OpenUIForm(string uiFormAssetName, string uiGroupName, int priority, bool pauseCoveredUIForm, object userData)
        {
            ...
            if (uiFormInstanceObject == null)
            {
                ...
                AssetManager.Instance.StartLoadGameObject(null, uiFormAssetName, OpenUIFormInfo.Create(serialId, uiGroup, pauseCoveredUIForm, userData), LoadGameObjectSuccessCallback);
            }
            else
            {
        ...
        public void CloseUIForm(int serialId, object userData)
        {
            ...
            if (uiForm == null)
            {
                throw new Exception(string.Format("Can not find UI form '{0}'.", serialId));
            }
        ...
        private void LoadGameObjectSuccessCallback(string path, Object asset, AssetObject assetObject)
        {
            OpenUIFormInfo openUIFormInfo = (OpenUIFormInfo)assetObject.userData;
            ...
            if (m_UIFormsToReleaseOnLoad.Contains(openUIFormInfo.SerialId))
            {
                ...
                m_UIFormHelper.ReleaseUIForm(asset, null);

                GameObject.Destroy(assetObject.gameObject);
                return;
            }

            m_UIFormsBeingLoaded.Remove(openUIFormInfo.SerialId);
            UIFormInstanceObject uiFormInstanceObject = UIFormInstanceObject.Create(path, asset, m_UIFormHelper.InstantiateUIForm(asset), m_UIFormHelper);
            m_InstancePool.Register(uiFormInstanceObject, true);

            assetObject.transform.SetParent(((GameObject)uiFormInstanceObject.Target).transform);

            InternalOpenUIForm(openUIFormInfo.SerialId, path, openUIFormInfo.UIGroup, uiFormInstanceObject.Target, openUIFormInfo.PauseCoveredUIForm, true, 0f, openUIFormInfo.UserData);
            ReferencePool.Release(openUIFormInfo);
        }
        ...
        //    m_UIFormsBeingLoaded.Remove(openUIFormInfo.SerialId);
        //    string appendErrorMessage = string.Format("Load UI form failure, asset name '{0}', status '{1}', error message '{2}'.", uiFormAssetName, status, errorMessage);
        //    if (m_OpenUIFormFailureEventHandler != null)
        //    {
