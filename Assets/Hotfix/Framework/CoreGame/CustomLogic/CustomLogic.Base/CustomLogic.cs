...
namespace LccHotfix
{
    ...
    /// 自定义逻辑静态配置，装配信息
    /// </summary>
    public class CustomLogicCfg : ICustomNodeCfg, ICustomLogicCfg
    {
        ...
        public CustomLogicCfg(int id, List<ICustomNodeCfg> nodeCfgList, System.Type logicType)
        {
            ID = id;
    ...
    public class CustomLogic : CustomNode, INeedUpdate, INeedStopCheck
    {
        ...
        private List<INeedUpdate> _needUpdateList = new List<INeedUpdate>();
        private List<INeedStopCheck> _needStopCheckList = new List<INeedStopCheck>();

        //自定义的子节点
        protected List<ICustomNode> _nodes;

        public CustomLogic()
        {
            _nodes = new List<ICustomNode>();
        }
        ...
        public override void InitializeNode(ICustomNodeCfg cfg, in CustomNodeContext context)
        {
            ...
            Inner_InitializeNodes(logicCfg, context, _usedTempLogicSet);
        }
        ...
        private void Inner_InitializeNodes(ICustomLogicCfg logicCfg, CustomNodeContext context, HashSet<int> usedTempLogicSet)
        {
            ...
            for (int i = 0; i < nodeCfgList.Count; i++)
            {
                ...
                CustomNode theNode = factory.CreateCustomNode(nodeCfg, context);
        ...
        public override void Destroy()
        {
            ...
            {
                factory.DestroyCustomNode(_nodes[i]);
            }
        ...
        public override void CollectInterfaceInChildren<T>(ref List<T> interfaceList)
        {
            for (int i = 0; i < _nodes.Count; ++i)
            {
                TraverseCollectInterface(ref interfaceList, _nodes[i]);
            }
        }

        protected void AddCustomNode(CustomNode node)
        {
            _nodes.Add(node);
            CacheInterface(node);
        }
        ...
        protected virtual void CacheInterface(CustomNode node)
        {
            ...
            {
                _needUpdateList.Add(updateNode);
            }

            TraverseCollectInterface(ref _needStopCheckList, node);
        }

        //清理interface
        protected virtual void ClearInterfaceCache()
        {
            _needUpdateList.Clear();
            _needStopCheckList.Clear();
        }
        ...
        {
            for (int i = 0; i < _needStopCheckList.Count; ++i)
            {
                var stopcheck = _needStopCheckList[i];
