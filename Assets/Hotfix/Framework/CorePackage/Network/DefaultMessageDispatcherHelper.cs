...
namespace LccHotfix
{
    public class DefaultMessageDispatcherHelper : IMessageDispatcherHelper
    {
        ...
        public void RegisterMessage(int code, Action<MessageObject> handle)
        {
            ...
            if (handleList != null)
            {
                ...
                {
                    Debug.Log($"存在相同的handle 消息号{code}");
                }
            }
        }
        ...
        public void DispatcherMessage(NetworkMessage message)
        {
            ...
            if (!_handleDict.TryGetValue(code, out var handleList))
            {
                Debug.LogWarning($"消息号{code} 没有处理");
                return;
            }
            ...
            for (int i = handleList.Count - 1; i >= 0; i--)
            {
                ...
                {
                    Debug.LogError($"消息号{code} 处理失败" + e.Message);
                }
            }
        }
    }
