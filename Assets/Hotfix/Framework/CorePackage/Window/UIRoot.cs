...
namespace LccHotfix
{
    ...
    {
        IDictionary<UILayerID, UILayer> _uiLayers = new Dictionary<UILayerID, UILayer>();
        IDictionary<string, UINode> _uiElements = new Dictionary<string, UINode>();
        ...
        {
            if (!_uiElements.TryGetValue(elementKey, out var element))
                return null;

            _uiElements.Remove(elementKey);
            element.DetachedFromRoot();
            return element;
        }
