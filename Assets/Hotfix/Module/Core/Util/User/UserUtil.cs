...
namespace LccHotfix
{
    public static class UserUtil
    {
        ...
        {
            string value = RijndaelUtil.RijndaelEncrypt("xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx", JsonMapper.ToJson(obj));
            FileUtil.SaveAsset(path, name, Encoding.UTF8.GetBytes(value));
            return value;
        }
        public static DataListInstance GetDataList(string path, string name)
        {
            string value = RijndaelUtil.RijndaelDecrypt("xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx", Encoding.UTF8.GetString(FileUtil.GetAsset(path, name)));
